...
function isInsideCircle({ center, radius }, { x, y }) {
  if (center.x === x) {
    return radius > Math.abs(center.y - y);
  }
  if (center.y === y) {
    return radius > Math.abs(center.x - x);
  }
  const cat1 = (center.x - x) ** 2;
  const cat2 = (center.y - y) ** 2;
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < set.length; i += 1) {
    if (str.indexOf(set[i]) === str.lastIndexOf(set[i]) && set[i] !== ' ') return set[i];
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  const assum = `${ccn}`.length % 2 === 0 ? 0 : 1;
  // eslint-disable-next-line no-nested-ternary
  const newArr = `${ccn}`.split('').map((item, index) => ((index + assum) % 2 === 0 ? (item * 2 > 9 ? item * 2 - 9 : item * 2) : item));
  return newArr.reduce((cum, item) => +cum + +item) % 10 === 0;
}
